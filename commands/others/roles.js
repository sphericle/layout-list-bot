...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("editrole")
        .setDescription("Manage custom role")
        ...
        .addStringOption((option) =>
            option.setName("icon").setDescription("A link to your role's icon")
        ),
    async execute(interaction) {
        ...
        if (!role) {
            return interaction.editReply(
                ":x: Could not find you custom role, ensure it isn't deleted and you still have it"
            );
        }
        try {
            await role.edit({
